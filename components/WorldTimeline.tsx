import React from 'react';
import { WorldEventData } from '../types';
import WorldTimelineEvent from './WorldTimelineEvent';

interface WorldTimelineProps {
  events: WorldEventData[];
}

const WorldTimeline: React.FC<WorldTimelineProps> = ({ events }) => {
  // NOTE: This uses the same scale as the main timeline for consistency.
  // In a future update, these timelines could be scrolled in sync.
  const REMS_PER_DAY = 3; 
  const HORIZONTAL_PADDING_REM = 50;

  const eventsWithDay = events.filter(e => e.day !== undefined);
  if (eventsWithDay.length === 0) {
    return null;
  }

  // To make the world timeline match the gospel timeline, we need a shared reference.
  // For now, we'll create a reasonable approximation.
  const minDay = -100; // A little before Christ's birth
  const maxDay = 17000; // A bit after the last world event
  const totalDays = maxDay - minDay;
  const timelineWidthRem = (totalDays * REMS_PER_DAY) + (HORIZONTAL_PADDING_REM * 2);

  return (
    <div className="w-full h-full p-4 overflow-x-auto overflow-y-hidden">
      <div className="relative h-full" style={{ width: `${timelineWidthRem}rem` }}>
        {/* The horizontal timeline bar */}
        <div className="absolute top-1/2 left-0 w-full h-0.5 bg-gray-600 -translate-y-1/2"></div>
        
        {events.map((event, index) => {
          const positionRem = ((event.day - minDay) * REMS_PER_DAY) + HORIZONTAL_PADDING_REM;
          return (
              <WorldTimelineEvent 
                  key={event.id} 
                  event={event} 
                  index={index} 
                  style={{ left: `${positionRem}rem` }}
              />
          );
        })}
      </div>
    </div>
  );
};

export default WorldTimeline;
